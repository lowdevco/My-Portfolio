  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="grid items-center justify-center grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-12 xl:gap-20 w-full py-24 relative z-10">
      ...
      <div className="order-2 lg:order-2 lg:col-span-7 text-center lg:text-left flex flex-col justify-center">
        <p className="text-[#A57AFF] font-mono text-sm md:text-base font-semibold tracking-widest uppercase mb-4">
          Behind the Code
        </p>
        <h2 className="text-4xl md:text-5xl lg:text-6xl font-extrabold text-[#f5f5f7] tracking-tight leading-tight mb-8">
        ...
        <div className="flex flex-wrap justify-center lg:justify-start gap-4 mt-12">
          {[
            { label: "Location", value: "Kerala, India", icon: <FiMapPin className="w-5 h-5" aria-hidden="true" /> },
            { label: "Experience", value: "Full Stack", icon: <FiTerminal className="w-5 h-5" aria-hidden="true" /> },
            { label: "Focus", value: "Web Apps", icon: <FiLayers className="w-5 h-5" aria-hidden="true" /> }
          ].map((stat, i) => (
